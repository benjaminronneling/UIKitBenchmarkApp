...
\begin{tikzpicture}
\begin{axis}[
    title=Plot of scenario \textbf{Text} on \textbf{\plotTitle}, % Set the title to the table name read from the CSV file
...
\addplot table [x=NumberOfComponents, y=TextMemory(UIKit), col sep=comma, meta=TextMemory(UIKit)] {\datatable};
\addlegendentry{UIKit}

\addplot table [x=NumberOfComponents, y=TextMemory(SwiftUI), col sep=comma, meta=TextMemory(SwiftUI)] {\datatable};
