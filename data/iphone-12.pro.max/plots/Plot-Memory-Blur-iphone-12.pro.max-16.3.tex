...
\begin{tikzpicture}
\begin{axis}[
    title=Plot of scenario \textbf{Blur} on \textbf{\plotTitle}, % Set the title to the table name read from the CSV file
...
\addplot table [x=NumberOfComponents, y=BlurMemory(UIKit), col sep=comma, meta=BlurMemory(UIKit)] {\datatable};
\addlegendentry{UIKit}

\addplot table [x=NumberOfComponents, y=BlurMemory(SwiftUI), col sep=comma, meta=BlurMemory(SwiftUI)] {\datatable};
