...
\begin{tikzpicture}
\begin{axis}[
    title=Plot of scenario \textbf{Images} on \textbf{\plotTitle}, % Set the title to the table name read from the CSV file
...
\addplot table [x=NumberOfComponents, y=ImagesMemory(UIKit), col sep=comma, meta=ImagesMemory(UIKit)] {\datatable};
\addlegendentry{UIKit}

\addplot table [x=NumberOfComponents, y=ImagesMemory(SwiftUI), col sep=comma, meta=ImagesMemory(SwiftUI)] {\datatable};
