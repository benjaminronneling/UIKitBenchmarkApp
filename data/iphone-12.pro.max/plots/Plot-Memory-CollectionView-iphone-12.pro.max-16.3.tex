...
\pgfplotstableread[col sep=comma]{data/Table-iphone-12.pro.max-16.3.csv}\datatable
...
\begin{tikzpicture}
\begin{axis}[
    title=Plot of scenario \textbf{CollectionView} on \textbf{\plotTitle}, % Set the title to the table name read from the CSV file
...
\addplot table [x=NumberOfComponents, y=CollectionViewMemory(UIKit), col sep=comma, meta=CollectionViewMemory(UIKit)] {\datatable};
\addlegendentry{UIKit}

\addplot table [x=NumberOfComponents, y=CollectionViewMemory(SwiftUI), col sep=comma, meta=CollectionViewMemory(SwiftUI)] {\datatable};
